...
public abstract class Card {
    ...
    public int chooseAPlayer(Player player,Board board){
        ...
        if (choosenIndex > board.getPlayers().size() || choosenIndex <= 0 || board.getPlayers().get(choosenIndex-1).getName().equals(player.getName())){
            while (choosenIndex > board.getPlayers().size() || choosenIndex <= 0 || board.getPlayers().get(choosenIndex-1).getName().equals(player.getName())){
